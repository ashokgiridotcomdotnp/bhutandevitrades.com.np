...
function getStartupWarnings() {
  ...
  if (config.app.isProduction && !toTrimmedString(process.env.TRUST_PROXY_HOPS)) {
    warnings.push('TRUST_PROXY_HOPS is not set. Defaulting to 1. Verify your reverse proxy setup to avoid spoofed client IP/protocol issues.');
  }
  ...
  if (
    ...
    warnings.push('Cloudinary credentials are not fully configured. Uploaded product images will be stored on the local filesystem.');
  }

  if (config.app.isProduction && !config.resend.apiKey) {
    warnings.push('RESEND_API_KEY is not configured. Email notifications will be disabled.');
  }
